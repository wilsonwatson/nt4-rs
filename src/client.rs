...
use crate::{client_config::Config, error::Error, message_type::Type, messages::{Announce, NTMessage, PublishTopic, SetProperties, Subscribe}, subscription::{InternalSub, MessageData, Subscription, SubscriptionData, SubscriptionOptions}, topic::{PublishProperties, PublishedTopic, Topic}};
...
struct InnerClient {
    ...
    socket_panic_receiver: parking_lot::Mutex<oneshot::Receiver<crate::error::Error>>,
...
impl Client {
    pub async fn try_new_w_config(
        ...
        let (panic_sender, panic_recv) = oneshot::channel::<crate::error::Error>();
    ...
    pub async fn try_new(server_addr: impl Into<SocketAddr>) -> Result<Self, crate::error::Error> {
        Self::try_new_w_config(server_addr, Config::default()).await
    }

    pub async fn new_w_config(server_addr: impl Into<SocketAddr>, config: Config) -> Result<Self, crate::error::Error> {
        Self::try_new_w_config(server_addr, config).await
    }

    pub async fn new(server_addr: impl Into<SocketAddr>) -> Result<Self, crate::error::Error> {
    ...
    ) -> Result<PublishedTopic, crate::error::Error> {
    ...
    pub async fn unpublish(&self, topic: PublishedTopic) -> Result<(), crate::error::Error> {
        // Put message in an array and serialize
    ...
    pub async fn unsubscribe(&self, sub: Subscription) -> Result<(), crate::error::Error> {
        // Put message in an array and serialize
...
impl InnerClient {
    ...
    fn check_task_panic(&self) -> Result<(), crate::error::Error> {
    ...
    pub(crate) async fn send_message(&self, message: Message) -> Result<(), crate::error::Error> {
    ...
    pub(crate) async fn update_time(&self) -> Result<(), crate::error::Error> {
    ...
    pub(crate) async fn on_open(&self) -> crate::error::Result<()> {
        ...
        self.send_message(Message::Text(serde_json::to_string(&messages)?))
            .await
            .ok();
        Ok(())
    }
...
async fn handle_message(client: Arc<InnerClient>, message: Message) -> crate::error::Result<()> {
    match message {
        Message::Text(message) => {
            // Either announce, unannounce, or properties
            let messages: Vec<NTMessage> =
                match serde_json::from_str(&message).map_err(Into::<crate::error::Error>::into) {
            ...
            for message in messages {
                match message {
                    NTMessage::Announce(Announce {
                        ...
                        (client.config.on_announce)(announced.get(&id).ok_or(crate::error::Error::Idk)?).await;
                    }
...
async fn setup_socket(
    ...
    panic_sender: oneshot::Sender<crate::error::Error>,
) -> Result<(), crate::error::Error> {
    let mut request = format!(
        "ws://{}/nt/watson-vision-{}",
        client.upgrade().ok_or(crate::error::Error::Idk)?.server_addr,
        client.upgrade().ok_or(crate::error::Error::Idk)?.id,
    )
    .into_client_request()?;
    ...
    let (mut socket, _) = tokio::time::timeout(
        Duration::from_millis(client.upgrade().ok_or(crate::error::Error::Idk)?.config.connect_timeout),
    ...
    tokio::spawn(async move {
        loop {
            let err: Result<(), crate::error::Error> = select! {
